...
namespace Polygon.Core
{
    public partial class MonteCarloAreaCalculator : AreaCalculator
    {
        ...
        public Task<double> CalculateAreaAsync(ReadOnlyMemory<Point> shape, CancellationToken cancellation, IProgress<AreaCalculationProgress> progress)
        {
            ...
            return Task.Run(() =>
            {
                ...
                double GetArea() => (maxX - minX) * (maxY - minY) * calcController.InCounter / calcController.Iterations;
                ...
                for (calcController.Start(), calcController.Report0Perc(); calcController.Continue; calcController.Iterations++)
                {
                    ...
                    calcController.ReportProgress(GetArea());
                }

                calcController.EnsureReported100Perc(GetArea());
                return GetArea();
            });
        }
